...
import net.mcreator.pangeaultima.procedures.GazelleRunProcedure;
import net.mcreator.pangeaultima.procedures.GazelleNaturalEntitySpawningConditionProcedure;
...
public class GazelleEntity extends Animal implements IAnimatable {
	private AnimationFactory factory = GeckoLibUtil.createFactory(this);
	private boolean swinging;
	private long lastSwing;
	public String animationprocedure = "empty";

	public GazelleEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(PangeaUltimaModEntities.GAZELLE.get(), world);
	}

	public GazelleEntity(EntityType<GazelleEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(3, new AvoidEntityGoal<>(this, CoyoteEntity.class, (float) 6, 1, 1.2));
		this.goalSelector.addGoal(4, new AvoidEntityGoal<>(this, HyenaEntity.class, (float) 6, 1, 1.2));
		this.goalSelector.addGoal(5, new AvoidEntityGoal<>(this, TigerEntity.class, (float) 6, 1, 1.2));
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(PangeaUltimaModItems.GAZELLE_CHOPS.get()));
	}
	...
	public void baseTick() {
		super.baseTick();
		GazelleRunProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
	}

	@Override
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		GazelleEntity retval = PangeaUltimaModEntities.GAZELLE.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(PangeaUltimaModEntities.GAZELLE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> {
					int x = pos.getX();
					int y = pos.getY();
					int z = pos.getZ();
					return GazelleNaturalEntitySpawningConditionProcedure.execute(world, x, y, z);
				});
	}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(GazelleEntity.RemovalReason.KILLED);
		}
	}
