...
public class HornbillEntity extends PathfinderMob implements IAnimatable {
	...
	public HornbillEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(PangeaUltimaModEntities.HORNBILL.get(), world);
	}

	public HornbillEntity(EntityType<HornbillEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(6, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = HornbillEntity.this.getRandom();
				double dir_x = HornbillEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = HornbillEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = HornbillEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = HornbillEntity.this.getX();
				double y = HornbillEntity.this.getY();
				double z = HornbillEntity.this.getZ();
				Entity entity = HornbillEntity.this;
				Level world = HornbillEntity.this.level;
				return super.canUse() && FlyProcedure.execute(entity);
			}
	...
	public SoundEvent getAmbientSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("pangea_ultima:trumpeter_hornbill"));
	}
	...
	public static void init() {
		SpawnPlacements.register(PangeaUltimaModEntities.HORNBILL.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(HornbillEntity.RemovalReason.KILLED);
		}
	}
